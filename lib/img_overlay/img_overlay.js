 ...
 * and handles both <img> (copies src/alt, or data-full when the page shows a
 * thumbnail) and inline <svg> (clones the node).
 *
 * Gestures:
...
const ImageOverlay = (() =>
{
  ...
  function open(el)
  {
    if (el.tagName === 'IMG') {
      ...
      // data-full lets a page show a light thumbnail and still zoom the full-res file
    ...
    // With data-full the shown file is still loading, so its real dimensions
    // (and with them the auto-flip decision) are only known on load
    if (mediaEl === overlayImg && !overlayImg.complete)
      overlayImg.addEventListener('load', layout, { once: true });
  ...
  /**
   * Size the media to the viewport and decide the auto-flip. Measures the
   * element that is actually shown (not the clicked thumbnail), so it must run
   * again once a lazily loaded full-res file knows its dimensions.
   */
  function layout()
  {
    if (!mediaEl)
    ...
    flipd = isMobile && isPortrait && getAspectRatio(mediaEl) >= 1.8;
