...
class VoiceAgent
{
  constructor()
  {
    ...
    // IMPORTANT: set to false in production (verbose logging adds jank on mobile).
    this.debug = false;

    // Simulate a silent microphone instead of using getUserMedia, so the
    // WebSocket/model handshake and AI greeting can be tested on a machine with
    // NO microphone (e.g. a PC). Leave false on machines that have a mic.
    this.simulateMic = false;
      ...
      this.ws.onopen = () => {
        ...
        const setupMessage = {
          setup: {
            ...
            systemInstruction: {
              parts: [{ text: this.sessionConfig.system_instruction + this.pageLanguageNote() }]
            },
