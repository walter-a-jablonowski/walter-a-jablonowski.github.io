...
const LOADING_CONFIG = {
  ...
  // Onboarding tooltip pointing at the hero "Use Case finder" button
  // (shown on every page load while true; never on mobile, to save space)
  showUseCaseTooltip: true,
  ...
  // Hide commercial content via body.hide-biz: prices/.biz-only + .service-cta
  // (the service-page FAQ stays visible - it backs the FAQPage schema)
  hideBiz: true
};
...
const VOICE_AGENT_CONFIG = {
  ...
  // The live conversation runs through the Worker relay (proxyUrl + '/live'):
  // the browser gets no API key, no prompt, no site guide and no provider
  // protocol. There is no longer a direct-to-Google mode to switch back to.

  // Text model for generateContent REST calls (use case finder).
  // Live/WebSocket models don't support generateContent.
  textModel: 'gemini-3.1-flash-lite',
  // textModel: 'gemini-3.5-flash',    // high demand
...
// Use Case Finder (sidebar view; uses VOICE_AGENT_CONFIG.proxyUrl + textModel)
const USE_CASE_FINDER_CONFIG = {

  // Use cases generated per round (initial search and each "Show more").
  // Also fills the {count} placeholder in the sidebar subtitle. Worker caps at 10.
  countPerLoad: 3,

  // How often the user may click "Show more" to generate additional use cases
  // (0 hides the button; each click costs one model call)
  maxShowMore: 1
};
