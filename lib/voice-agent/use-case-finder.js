...
class UseCaseFinder
{
  ...
  _inquire()
  {
    ...
    // Blank lines after the labels and after each entry headline: the mail is
    // read as plain text, where the gaps are what separates the blocks
    const lines = [];
    ...
    const inquiry = {
      subject: this._plain( this.t.inquirySubject ),
      message: this._plain( lines.join( '\n' ) ),
      lang: this.lang
    };
  ...
  _prefillContactForm( inquiry )
  {
    ...
    if( messageField )
    {
      ...
      // Give the field some room for the use cases (see WebsiteController.
      // fitTextarea). Guarded: the finder must keep working on its own if the
      // site controller is ever absent.
      if( typeof WebsiteController !== 'undefined' )
        WebsiteController.fitTextarea( messageField );
    }
  }

  /**
   * HTML source string → plain text (&auml; → ä).
   *
   * The label strings are written with entities because most of them are
   * rendered into HTML. The mail body and the downloaded .txt are plain text,
   * where an entity would show up literally — so everything leaving through
   * those two paths goes through here. A detached <textarea> parses its content
   * as raw text, so this only resolves entities; no markup is created.
   */
  _plain( str )
  {
...
<body>
  ...
  _download()
  {
    ...
    // Decoded up front: both are underlined to their own width, which an entity
    // would count wrong (&auml; is 6 characters, ä is one)
    const title  = this._plain( this.t.docTitle );
    const header = this._plain( this.t.resultsHeader );
    ...
    const blob = new Blob( [ this._plain( lines.join( '\n' ) ) ], { type: 'text/plain;charset=utf-8' } );
...
(function()
{
  function init()
  {
    ...
    // A page may offer the finder in more than one place (hero button, section heading)

    document.querySelectorAll('.use-case-finder-trigger').forEach( trigger => {

      // The page declares its topic on the trigger; primes the placeholder
      if( trigger.dataset.ucfTopic )
        finder.topic = trigger.dataset.ucfTopic;
