...
class VoiceAgent
{
  ...
  navInstructionNote()
  {
    ...
    const multi = ' If several places are relevant, pass them all in one call (targets array) so the ' +
                  'visitor sees a short list and can pick.';
    ...
    return '\n\n# Navigation\nYou can point visitors to the right place with the `navigate` tool ' +
           ...
           'them the link and tell them they can click it. Do NOT claim you will move the page yourself.' + multi;
  }
  ...
  handleToolCall( toolCall )
  {
    ...
    calls.forEach((fc) => {
      if( fc.name === 'navigate' ) {
        // Accept the new `targets` array; tolerate a legacy single `target`.
        const raw = (fc.args && (fc.args.targets || fc.args.target)) || [];
        const ids = (Array.isArray(raw) ? raw : [raw])
          .filter(id => this.navTargets().some(t => t.id === id));
        if( ids.length > 1 )
          // Several options: always let the visitor choose, even in auto mode.
          this.renderNavLinkList(ids);
        else if( ids.length === 1 ) {
          if( this.navMode === 'auto' )
            this.autoNavigate(ids[0]);
          else
            this.renderNavLink(ids[0]);
        }
      }
   ...
   * option). Invalid ids are silently dropped upstream in handleToolCall.
   */
  renderNavLinkList( ids )
  {
    const targets = ids
      ...
      .filter(Boolean);
    if( targets.length < 2 || ! this.linkSlot ) {
