...
class UseCaseFinder
{
  ...
  async _showMore()
  {
    ...
    const btn = this.sidebar.contentEl.querySelector( '#ucf-more' );
    if( ! btn )
      return;

    const oldError = this.sidebar.contentEl.querySelector( '.ucf-more-error' );
    if( oldError )
      oldError.remove();
  ...
  _renderResults( useCases )
  {
    ...
    const moreHtml = this._moreCount < this.maxShowMore
      ? `<div class="ucf-more-row">
           <button type="button" class="ucf-more-btn" id="ucf-more"><i class="fas fa-plus"></i>${this.t.showMore}</button>
         </div>`
      : '';
  ...
  _bindSelectionEvents()
  {
    ...
    const moreBtn = this.sidebar.contentEl.querySelector( '#ucf-more' );
    if( moreBtn )
